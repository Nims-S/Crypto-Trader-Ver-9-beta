...
class BitunixAdapter(BaseExchangeAdapter):
    ...
    async def submit_order(self, event: OrderSubmitted) -> None:
        ...
        await self.publish_order_accepted(
            order_id=event.order_id,
            exchange_order_id=f"bitunix-{uuid4()}",
            correlation_id=event.correlation_id,
        )
    ...
    def normalize_trade_payload(
        ...
        *,
        ...
        return TradeEvent(
            ...
            symbol=self._string_value(payload, "symbol", "market", default="UNKNOWN"),
            price=self._float_value(payload, "price", "last", "lastPrice"),
            quantity=self._float_value(payload, "quantity", "volume", "baseVolume"),
            side=self._string_value(payload, "side", default="unknown").lower(),
        )
    ...
    def normalize_fill_payload(
        ...
        *,
        ...
        return FillReceived(
            ...
            timestamp_ns=self._timestamp_ns(),
            correlation_id=correlation_id,
            order_id=self._string_value(payload, "order_id", "clientOrderId", "orderId"),
            fill_id=self._string_value(payload, "fill_id", "tradeId", "execId"),
            price=self._float_value(payload, "price", "fillPrice"),
            quantity=self._float_value(payload, "quantity", "fillQty", "qty"),
            fee=self._float_value(payload, "fee", "feeAmount", default=0.0),
            fee_asset=self._string_value(payload, "fee_asset", "feeCoin", default="USDT"),
        )
