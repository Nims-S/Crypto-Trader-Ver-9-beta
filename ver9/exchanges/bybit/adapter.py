...
class BybitAdapter(BaseExchangeAdapter):
    def __init__(self, *, event_bus: EventBus) -> None:
        super().__init__(exchange_name="bybit", event_bus=event_bus)
    ...
    async def submit_order(self, event: OrderSubmitted) -> None:
        ...
        await self.publish_order_accepted(
            order_id=event.order_id,
            exchange_order_id=f"bybit-{uuid4()}",
            correlation_id=event.correlation_id,
        )
    ...
    def normalize_trade_payload(
        ...
        *,
        ...
        return TradeEvent(
            ...
            quantity=self._float_value(payload, "quantity", "q", "size"),
    ...
    def normalize_fill_payload(
        ...
        *,
        ...
        return FillReceived(
            ...
            timestamp_ns=self._timestamp_ns(),
            correlation_id=correlation_id,
            order_id=self._string_value(payload, "order_id", "orderLinkId", "orderId"),
            fill_id=self._string_value(payload, "fill_id", "execId", "tradeId"),
            price=self._float_value(payload, "price", "execPrice"),
            quantity=self._float_value(payload, "quantity", "execQty", "qty"),
            fee=self._float_value(payload, "fee", "execFee", default=0.0),
            fee_asset=self._string_value(payload, "fee_asset", "feeCurrency", default="USDT"),
        )
